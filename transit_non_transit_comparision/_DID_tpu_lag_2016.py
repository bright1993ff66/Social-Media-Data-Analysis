...
def add_post_variable_lag_effect(string, opening_start_date, opening_end_date, lag_effect_month=0):
    ...
    if lag_effect_month == np.inf:
        ...
        # For the 'after period', we always consider 1-year tweet data
...
def get_population_one_area_seperate(dataframe: pd.DataFrame, tpu_info_dataframe: pd.DataFrame):
    """
    Get the population data based on treatment and control setting
    :param dataframe: a pandas dataframe containing the data for DID analysis
    :param tpu_info_dataframe: pandas dataframe saving the census information
    :return: dataframe containing the population
    """
    assert 'T_i_t' in dataframe, 'The dataframe should have treatment and control indicator'
    assert 'population_2016' in tpu_info_dataframe, 'The census data should have population information in 2016'
    assert 'SmallTPU' in tpu_info_dataframe, "The census data should contain the TPU information"
    census_dict = pd.Series(tpu_info_dataframe.population_2016.values, index=tpu_info_dataframe.SmallTPU).to_dict()
    dataframe['Population'] = dataframe.apply(lambda row: census_dict[row['TPU']], axis=1)
    dataframe['Population_log'] = dataframe.apply(lambda row: np.log(census_dict[row['TPU']]), axis=1)
    return dataframe
...
def get_median_income_one_area_seperate(dataframe: pd.DataFrame, tpu_info_dataframe: pd.DataFrame):
    """
    ...
    """
    assert 'T_i_t' in dataframe, 'The dataframe should have treatment and control indicator'
    assert 'm_income_2016' in tpu_info_dataframe, 'The census data should have median income information in 2016'
    assert 'SmallTPU' in tpu_info_dataframe, "The census data should contain the TPU information"
    census_dict = pd.Series(tpu_info_dataframe.m_income_2016.values, index=tpu_info_dataframe.SmallTPU).to_dict()
    dataframe['Median_Income'] = dataframe.apply(lambda row: census_dict[row['TPU']], axis=1)
    dataframe['Median_Income_log'] = dataframe.apply(lambda row: np.log(census_dict[row['TPU']]), axis=1)
    return dataframe
...
def build_regress_data_three_areas_seperate(kwun_tong_treatment, kwun_tong_control, south_horizons_treatment,
    ...
    result_dataframe_copy['Population_log'] = np.log(np.array(population_list))
    result_dataframe_copy['Median_Income'] = median_income_list
    result_dataframe_copy['Median_Income_log'] = np.log(np.array(median_income_list))
    return result_dataframe_copy


def remove_not_considered_data(dataframe):
    """
    Remove some data, including the following cases:
    1) some areas are missing
    2) at least one treatment TPU and control TPU of each study area should be included in the DID analysis
    :param dataframe: a pandas dataframe saving the population, income, tweet activity, tweet sentiment, etc
    :return: a filtered dataframe
    """
    assert 'Time' in dataframe, "The dataframe should contain the time information"
    assert 'Area_name' in dataframe, "The dataframe should contain the studied area name"
    assert 'T_i_t' in dataframe, "The dataframe should contain the treatment or control information"
    time_counter = Counter(dataframe['Time'])
    not_considered_time = []
    for time in time_counter:
        dataframe_select = dataframe.loc[dataframe['Time'] == time]
        area_set = set(dataframe_select['Area_name'])
        decision_1 = (len(area_set) == 3)
        decision_2 = True
        for area in area_set:
            dataframe_select_area = dataframe_select.loc[dataframe_select['Area_name'] == area]
            if len(set(dataframe_select_area['T_i_t'])) != 2:  # Make sure both treatment and control are included
                decision_2 = False
                break
        if not (decision_1 & decision_2):
            not_considered_time.append(time)
    print('Not considered time: {}'.format(not_considered_time))
    result_dataframe_final = dataframe.loc[~dataframe['Time'].isin(not_considered_time)].reset_index(drop=True)
    return result_dataframe_final
...
def build_regress_dataframe_for_one_station_seperate(treatment_dataframe, control_dataframe,
    ...
    activity_dict_log = {}
    for _, dataframe in combined_data_copy.groupby(['TPU_cross_sectional', 'month_plus_year', 'T_i_t', 'Post']):
        tpu = str(list(dataframe['TPU_cross_sectional'])[0])
        time = str(list(dataframe['month_plus_year'])[0])
        t_i_t = str(list(dataframe['T_i_t'])[0])
        post = str(list(dataframe['Post'])[0])
        sentiment_dict[time + '+' + t_i_t + '+' + post + '+' + tpu] = pos_percent_minus_neg_percent(dataframe)
        activity_dict[time + '+' + t_i_t + '+' + post + '+' + tpu] = dataframe.shape[0]
        activity_dict_log[time + '+' + t_i_t + '+' + post + '+' + tpu] = np.log(dataframe.shape[0])
    result_dataframe_copy = result_dataframe.copy()
    t_i_t_list = []
    tpu_list = []
    time_list, post_list = [], []
    ...
    dataframe_with_population = get_population_one_area_seperate(result_dataframe_copy,
                                                                 tpu_info_dataframe=tpu_info_dataframe)
    final_dataframe = get_median_income_one_area_seperate(dataframe_with_population,
                                                          tpu_info_dataframe=tpu_info_dataframe)
...
def conduct_combined_did_analysis(kwun_tong_treatment_data, kwun_tong_control_data,
    ...
    longitudinal_dataframe = build_regress_data_three_areas_seperate(
    ...
    if for_sentiment:
        # Only consider the time when more than 10 tweets were posted
        longitudinal_dataframe_activity_over_ten = longitudinal_dataframe.loc[longitudinal_dataframe['Activity'] >= 10]
        longitudinal_dataframe_for_sentiment = remove_not_considered_data(longitudinal_dataframe_activity_over_ten)
        print('{}% rows are selected for DID sentiment analysis'.format(
            longitudinal_dataframe_for_sentiment.shape[0] * 100 / longitudinal_dataframe.shape[0]))
        longitudinal_dataframe_for_sentiment.to_csv(os.path.join(did_data_saving_path, filename))
        ...
        reg_combined_sentiment = smf.ols(
            'Sentiment ~ T_i_t:Post+T_i_t+Post+Population_log+Median_Income_log+C(Area_name)',
            longitudinal_dataframe_for_sentiment).fit()
        ...
        longitudinal_dataframe_activity_over_ten = longitudinal_dataframe.loc[longitudinal_dataframe['Activity'] >= 10]
        longitudinal_dataframe_for_activity = remove_not_considered_data(longitudinal_dataframe_activity_over_ten)
        longitudinal_dataframe_for_activity.to_csv(os.path.join(did_data_saving_path, filename))
        ...
        reg_combined_activity = smf.ols(
            'log_Activity ~ T_i_t:Post+T_i_t+Post+Population_log+Median_Income_log+C(Area_name)',
            longitudinal_dataframe_for_activity).fit()
...
def conduct_did_analysis_one_area(treatment_considered_dataframe, control_considered_dataframe, opening_start_date,
    ...
    constructed_dataframe = build_regress_dataframe_for_one_station_seperate(
    ...
    constructed_dataframe_for_sent = constructed_dataframe.loc[constructed_dataframe['Activity'] >= 10]
    constructed_dataframe_for_act = constructed_dataframe.loc[constructed_dataframe['Activity'] >= 10]
    combined_sentiment = smf.ols(
        'Sentiment ~ T_i_t:Post+T_i_t+Post', constructed_dataframe_for_sent).fit()
    combined_activity = smf.ols(
        'log_Activity ~ T_i_t:Post+T_i_t+Post', constructed_dataframe_for_act).fit()
