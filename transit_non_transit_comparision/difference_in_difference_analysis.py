...
def add_post_variable(string, opening_start_date, opening_end_date, check_window=0):
    ...
    if check_window == 0:
        ...
        # print('===================================================')
        # print('The left time range is: {}'.format(final_left_range))
        # print('The right time range is: {}'.format(right_time_range))
        # print('===================================================')
...
def build_regress_dataframe_for_combined_areas(kwun_tong_treatment, kwun_tong_control_sentiment,
                                               kwun_tong_control_activity, south_horizons_treatment,
    ...
    :param kwun_tong_control_sentiment: the dataframe saving tweets for kwun tong control area for sentiment DID
    :param kwun_tong_control_activity: the dataframe saving tweets for kwun tong control area for activity DID
    ...
    kwun_tong_control_sentiment['T_i_t'] = [0] * kwun_tong_control_sentiment.shape[0]
    kwun_tong_control_activity['T_i_t'] = [0] * kwun_tong_control_activity.shape[0]
    ...
    kwun_tong_control_sentiment['Post'] = kwun_tong_control_sentiment.apply(
        lambda row: add_post_variable(row['hk_time'], opening_start_date=october_23_start,
                                      opening_end_date=october_23_end, check_window=check_window_value), axis=1)
    kwun_tong_control_activity['Post'] = kwun_tong_control_activity.apply(
        lambda row: add_post_variable(row['hk_time'], opening_start_date=october_23_start,
                                      opening_end_date=october_23_end, check_window=check_window_value), axis=1)
    ...
    if sentiment_did:
        dataframe_list = [kwun_tong_treatment, kwun_tong_control_sentiment, south_horizons_treatment,
                          south_horizons_control, ocean_park_treatment, ocean_park_control]
    else:
        dataframe_list = [kwun_tong_treatment, kwun_tong_control_activity, south_horizons_treatment,
                          south_horizons_control, ocean_park_treatment, ocean_park_control]
...
def build_regress_dataframe_for_kwun_tong_line_extension(treatment_dataframe, control_dataframe_sentiment,
                                                         control_dataframe_activity, station_open_start_date,
                                                         station_open_end_date, open_year_plus_month,
                                                         check_window_value=0):
    # check the date
    assert open_year_plus_month in ['2016_10', '2016_12']
    result_dataframe = pd.DataFrame(columns=['Time', 'T_i_t', 'Post', 'Sentiment', 'Activity'])
    # build the T_i_t variable
    treatment_dataframe['T_i_t'] = [1] * treatment_dataframe.shape[0]
    control_dataframe_sentiment['T_i_t'] = [0] * control_dataframe_sentiment.shape[0]
    control_dataframe_activity['T_i_t'] = [0] * control_dataframe_activity.shape[0]
    ...
    print('For treatment area, check the post variable distribution of treatment group: {}'.format(
        Counter(treatment_dataframe['Post'])))
    print('For treatment area, check the T_i_t variable distribution of treatment group: {}'.format(
        Counter(treatment_dataframe['T_i_t'])))
    control_dataframe_sentiment['Post'] = control_dataframe_sentiment.apply(
        lambda row: add_post_variable(row['hk_time'], opening_start_date=station_open_start_date,
                                      opening_end_date=station_open_end_date, check_window=check_window_value), axis=1)
    print('For control area sentiment, check the post variable distribution of control group(sentiment): {}'.format(
        Counter(control_dataframe_sentiment['Post'])))
    print('For control area sentiment, check the T_i_t variable distribution of control group(sentiment): {}'.format(
        Counter(control_dataframe_sentiment['T_i_t'])))
    control_dataframe_activity['Post'] = control_dataframe_activity.apply(
        lambda row: add_post_variable(row['hk_time'], opening_start_date=station_open_start_date,
                                      opening_end_date=station_open_end_date, check_window=check_window_value), axis=1)
    print('For control area activity, check the post variable distribution of control group(activity): {}'.format(
        Counter(control_dataframe_activity['Post'])))
    print('For control area activity, check the T_i_t variable distribution of control group(activity): {}'.format(
        Counter(control_dataframe_activity['T_i_t'])))

    combined_dataframe_sentiment = pd.concat([treatment_dataframe, control_dataframe_sentiment], axis=0, sort=True)
    combined_dataframe_activity = pd.concat([treatment_dataframe, control_dataframe_activity], axis=0, sort=True)
    combined_dataframe_sentiment = combined_dataframe_sentiment.reset_index(drop=True)
    combined_dataframe_activity = combined_dataframe_activity.reset_index(drop=True)
    # We don't consider the tweets posted on the open date
    combined_dataframe_without_not_considered_sentiment = \
        combined_dataframe_sentiment.loc[combined_dataframe_sentiment['Post'] != 'not considered']
    combined_dataframe_without_not_considered_activity = \
        combined_dataframe_activity.loc[combined_dataframe_activity['Post'] != 'not considered']
    combined_data_copy_sentiment = combined_dataframe_without_not_considered_sentiment.copy()
    combined_data_copy_activity = combined_dataframe_without_not_considered_activity.copy()
    combined_data_copy_sentiment['month_plus_year'] = combined_data_copy_sentiment.apply(
        lambda row: str(int(float(row['year']))) + '_' + str(int(float(row['month']))), axis=1)
    combined_data_copy_activity['month_plus_year'] = combined_data_copy_activity.apply(
        lambda row: str(int(float(row['year']))) + '_' + str(int(float(row['month']))), axis=1)
    sentiment_dict = {}
    activity_dict = {}
    for _, dataframe in combined_data_copy_sentiment.groupby(['month_plus_year', 'T_i_t', 'Post']):
        time_sentiment = str(list(dataframe['month_plus_year'])[0])
        t_i_t_sentiment = str(list(dataframe['T_i_t'])[0])
        post_sentiment = str(list(dataframe['Post'])[0])
        sentiment_dict[time_sentiment + '_' + t_i_t_sentiment + '_' + post_sentiment] = \
            before_and_after_final_tpu.pos_percent_minus_neg_percent(
            dataframe)
    for _, dataframe in combined_data_copy_activity.groupby(['month_plus_year', 'T_i_t', 'Post']):
        time_activity = str(list(dataframe['month_plus_year'])[0])
        t_i_t_activity = str(list(dataframe['T_i_t'])[0])
        post_activity = str(list(dataframe['Post'])[0])
        activity_dict[time_activity + '_' + t_i_t_activity + '_' + post_activity] = np.log(dataframe.shape[0])
    result_dataframe_copy = result_dataframe.copy()
    time_list = []
    t_i_t_list = []
    post_list = []
...
def conduct_combined_did_analysis(kwun_tong_treatment_dataframe, kwun_tong_control_dataframe_sentiment,
                                  kwun_tong_control_dataframe_activity,
    ...
    longitudinal_dataframe = build_regress_dataframe_for_combined_areas(
        ...
        kwun_tong_control_sentiment=kwun_tong_control_dataframe_sentiment,
        kwun_tong_control_activity=kwun_tong_control_dataframe_activity,
...
def conduct_did_analysis_one_area(treatment_considered_dataframe, control_considered_dataframe, opening_start_date,
    ...
    constructed_dataframe_selected.to_csv(os.path.join(file_path, filename), encoding='utf-8')
    combined_sentiment = smf.ols('Sentiment ~ T_i_t+Post+T_i_t:Post', constructed_dataframe_selected).fit()
    combined_activity = smf.ols('Activity ~ T_i_t+Post+Post:T_i_t:Post', constructed_dataframe_selected).fit()
    print('----The sentiment did result-----')
...
def conduct_did_analysis_kwun_tong(treatment_considered_dataframe, control_dataframe_sentiment,
                                   control_dataframe_activity, opening_start_date,
                                  opening_end_date, open_year_month, window_size_value, file_path, filename):
    constructed_dataframe = build_regress_dataframe_for_kwun_tong_line_extension(
        treatment_dataframe=treatment_considered_dataframe,
        control_dataframe_sentiment=control_dataframe_sentiment,
        control_dataframe_activity=control_dataframe_activity,
        station_open_start_date=opening_start_date,
        station_open_end_date=opening_end_date, open_year_plus_month=open_year_month,
        check_window_value=window_size_value)
...
if __name__ == '__main__':
    ...
    kwun_tong_line_control_tpu_set_1000 = {'247', '234', '242', '212', '235'}
    kwun_tong_line_control_tpu_set_1500 = {'212', '246', '232', '228', '235', '242', '233', '211', '214', '247', '220',
                                           '229', '225', '234', '227'}
    ...
    kwun_tong_line_control_dataframe_sentiment = build_dataframe_based_on_set(datapath=path,
                                                                tpu_set=kwun_tong_line_control_tpu_set_1500)
    kwun_tong_line_control_dataframe_activity = build_dataframe_based_on_set(datapath=path,
                                                                tpu_set=kwun_tong_line_control_tpu_set_1000)
    print('For Kwun Tong Line control sentiment...')
    utils.number_of_tweet_user(kwun_tong_line_control_dataframe_sentiment)
    ...
    print('Overall Treatment and Control Comparison for sentiment(4 months)...')
    conduct_combined_did_analysis(kwun_tong_treatment_dataframe=kwun_tong_line_treatment_dataframe,
                                  kwun_tong_control_dataframe_sentiment=kwun_tong_line_control_dataframe_sentiment,
                                  kwun_tong_control_dataframe_activity=kwun_tong_line_control_dataframe_activity,
                                  ...
                                  filename='longitudinal_did_dataframe_4_months_sentiment.csv', check_window_value=4,
                                  for_sentiment=True)

    print('Overall Treatment and Control Comparison for activity(4 months)...')
    conduct_combined_did_analysis(kwun_tong_treatment_dataframe=kwun_tong_line_treatment_dataframe,
                                  kwun_tong_control_dataframe_sentiment=kwun_tong_line_control_dataframe_sentiment,
                                  kwun_tong_control_dataframe_activity=kwun_tong_line_control_dataframe_activity,
                                  south_horizons_treatment_dataframe=south_horizons_lei_tung_treatment_dataframe,
                                  ...
                                  ocean_park_control_dataframe=ocean_park_wong_chuk_hang_control_dataframe,
                                  dataframe_saving_path=dataframe_saving_path,
                                  filename='longitudinal_did_dataframe_4_months_activity.csv', check_window_value=4,
                                  for_sentiment=False)

    print('Overall Treatment and Control Comparison for sentiment(5 months)...')
    conduct_combined_did_analysis(kwun_tong_treatment_dataframe=kwun_tong_line_treatment_dataframe,
                                  kwun_tong_control_dataframe_sentiment=kwun_tong_line_control_dataframe_sentiment,
                                  kwun_tong_control_dataframe_activity=kwun_tong_line_control_dataframe_activity,
                                  south_horizons_treatment_dataframe=south_horizons_lei_tung_treatment_dataframe,
                                  ...
                                  ocean_park_control_dataframe=ocean_park_wong_chuk_hang_control_dataframe,
                                  dataframe_saving_path=dataframe_saving_path,
                                  filename='longitudinal_did_dataframe_5_months_sentiment.csv',
                                  check_window_value=5, for_sentiment=True)

    print('Overall Treatment and Control Comparison for activity(5 months)...')
    conduct_combined_did_analysis(kwun_tong_treatment_dataframe=kwun_tong_line_treatment_dataframe,
                                  kwun_tong_control_dataframe_sentiment=kwun_tong_line_control_dataframe_sentiment,
                                  kwun_tong_control_dataframe_activity=kwun_tong_line_control_dataframe_activity,
                                  south_horizons_treatment_dataframe=south_horizons_lei_tung_treatment_dataframe,
                                  ...
                                  ocean_park_control_dataframe=ocean_park_wong_chuk_hang_control_dataframe,
                                  dataframe_saving_path=dataframe_saving_path,
                                  filename='longitudinal_did_dataframe_5_months_activity.csv',
                                  check_window_value=5, for_sentiment=False)
    ...
    print('For 4 months....')
    conduct_did_analysis_kwun_tong(treatment_considered_dataframe=kwun_tong_line_treatment_dataframe,
                                   control_dataframe_sentiment=kwun_tong_line_control_dataframe_sentiment,
                                   control_dataframe_activity=kwun_tong_line_control_dataframe_activity,
